...
RCRA_Clean1 <- RCRA_Clean %>%
  ...
  mutate(Dataset = "Corrective Action Site Screening - RCRA",
        ...
        `Sample ID` = paste0("RCRA_",row_number())) |> 
...
# 6: Subset by media
RCRA_Groundwater_2025 <- RCRA_Clean4 %>% filter(Medium=="Groundwater")
RCRA_Leachate_2025 <- RCRA_Clean4 %>% filter(Medium=="Leachate")
...
library("writexl")
write_xlsx(RCRA_Groundwater_2025,"03_Clean_Data/Groundwater/RCRA_Groundwater_2025.xlsx")

write_xlsx(RCRA_Leachate_2025, "03_Clean_Data/Leachate/RCRA_Leachate_2025.xlsx")
