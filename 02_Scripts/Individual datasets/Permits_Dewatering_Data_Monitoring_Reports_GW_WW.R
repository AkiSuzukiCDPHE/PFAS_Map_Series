...
unique(DMR_RawData_4$PFAS_Analyte)
...
DMR_RawData_4 <- DMR_RawData_3 %>%
  mutate(
    PFAS_Analyte = case_when(
      Parameter %in% c("Perfluorooctanesulfonamide (PFOSA)") ~ "PFOSA",
      Parameter %in% c("Perfluoroheptanoic acid (PFHpA)") ~ "PFHpA",
      Parameter %in% c("Perfluorobutanesulfonic acid (PFBS)") ~ "PFBS",
      Parameter %in% c("Perfluorododecanoic acid (PFDoA)") ~ "PFDoA",
      Parameter %in% c("N-methyl perfluorooctanesulfonamide (NMeFOSA)") ~ "NMeFOSA",
      Parameter %in% c("Perfluorononanoic acid (PFNA)") ~ "PFNA",
      Parameter %in% c("Perfluoroundecanoic acid (PFUnA)") ~ "PFUnA",
      Parameter %in% c("N-ethyl perfluorooctanesulfonamide (NEtFOSA)") ~ "NEtFOSA",
      Parameter %in% c("Perfluorohexanoic acid (PFHxA)") ~ "PFHxA",
      Parameter %in% c("Perfluorotridecanoic acid (PFTrDA)") ~ "PFTrDA",
      Parameter %in% c("Perfluorononanesulfonic acid (PFNS)") ~ "PFNS",
      Parameter %in% c("Perfluorotetradecanoic acid (PFTeDA)") ~ "PFTeDA",
      Parameter %in% c("Perfluoropentanesulfonic acid (PFPeS)") ~ "PFPeS",
      Parameter %in% c("Perfluoroheptanesulfonic acid (PFHpS)") ~ "PFHpS",
      Parameter %in% c("4,8-Dioxa-3H-perfluorononanoic acid (ADONA)") ~ "ADONA",
      Parameter %in% c("Hexafluoropropylene oxide dimer acid (HFPO-DA)") ~ "HFPO-DA",
      Parameter %in% c("Perfluorodecanoic acid (PFDA)") ~ "PFDA",
      Parameter %in% c("Perfluoropentanoic acid (PFPeA)") ~ "PFPeA",
      Parameter %in% c("Perfluorooctanoic acid (PFOA)", "Ammonium perfluorooctanoate") ~ "PFOA",
      Parameter %in% c("Perfluorohexanesulfonic acid (PFHxS)", "Perfluorohexane Sulfonate") ~ "PFHxS",
      Parameter %in% c("N-ethyl perfluorooctanesulfonamidoacetic acid (NEtFOSAA)") ~ "NEtFOSAA",
      Parameter %in% c("Perfluorodecanesulfonic acid (PFDS)") ~ "PFDS",
      Parameter %in% c("Perfluorododecanesulfonic acid (PFDoS)") ~ "PFDoS",
      Parameter %in% c("Perfluorooctanesulfonic acid (PFOS)", "Perfluorooctanesulfonate") ~ "PFOS",
      Parameter %in% c("N-methyl perfluorooctanesulfonamidoacetic acid (NMeFOSAA)") ~ "NMeFOSAA",
      Parameter %in% c("Nonafluoro-3,6-dioxaheptanoic acid (NFDHA)") ~ "NFDHA",
      Parameter %in% c("Perfluorobutanoic acid (PFBA)") ~ "PFBA",
      Parameter %in% c("Perfluoro(2-ethoxyethane)sulfonic acid (PFEESA)") ~ "PFEESA",
      Parameter %in% c("9-Chlorohexadecafluoro-3-oxanonane-1-sulfonic acid (9Cl-PF3ONS)", 
                       "Potassium 9-chlorohexadecafluoro-3-oxanonane-1-sulfonate") ~ "9Cl-PF3ONS",
      Parameter %in% c("Perfluoro-4-methoxybutanoic acid (PFMBA)") ~ "PFMBA",
      Parameter %in% c("Perfluoro-2-methoxypropanoic acid") ~ "PFMOPrA",
      Parameter %in% c("Perfluoro-3-methoxypropanoic acid (PFMPA)") ~ "PFMPA",
      ...
      Parameter %in% c("1H,1H,2H,2H-Perfluorohexane sulfonic acid (4:2 FTS)") ~ "4:2 FTS",
      Parameter %in% c("1H,1H,2H,2H-Perfluorooctane sulfonic acid (6:2 FTS)") ~ "6:2 FTS",
      Parameter %in% c("1H,1H, 2H, 2H-Perfluorodecane sulfonic acid (8:2 FTS)") ~ "8:2 FTS",
      Parameter %in% c("3-Perfluoropropyl propanoic acid (3:3 FTCA)") ~ "3:3 FTCA",
      Parameter %in% c("N-methyl perfluorooctanesulfonamidoethanol (NMeFOSE)") ~ "NMeFOSE",
      Parameter %in% c("N-ethyl perfluorooctanesulfonamidoethanol (NEtFOSE)") ~ "NEtFOSE",
      Parameter %in% c("3-Perfluoroheptyl propanoic acid (7:3 FTCA)") ~ "7:3 FTCA",
      Parameter %in% c("2H,2H,3H,3H-Perfluorooctanoic acid (5:3 FTCA)") ~ "5:3 FTCA",
      TRUE ~ Parameter
    )
  )
...
Zeroes <- DMR_RawData_6 %>% filter(DMR_RawData_5$Concentration_3 == "0")
...
Zeroes_2 <- DMR_RawData_7 %>% filter(DMR_RawData_6$Concentration_2 == "0" |
...
NAValues <- DMR_RawData_9 %>% filter(is.na(Concentration_2) |
                                       is.na(Concentration_3))

# Convert Concentration_2 and # Concentration_3 to numeric
DMR_RawData_9$Concentration_2 <- as.numeric(DMR_RawData_7$Concentration_2)
DMR_RawData_9$Concentration_3 <- as.numeric(DMR_RawData_7$Concentration_3)
...
# Change ug/L and mg/L units to ng/L to standardize units
DMR_RawData_10 <- DMR_RawData_9.5  %>% mutate(
  Result = case_when(Concentration_Units == "mg/L" ~ Result*1000000,
                      Concentration_Units == "ug/L" ~ Result*1000, 
                      TRUE ~ Result))



DMR_RawData_10.5 <- DMR_RawData_10  %>% mutate(
...
DMR_RawData_11 <- DMR_RawData_10.5 %>%
...
wide_data_DMR3 <- wide_data_DMR2 %>% mutate(`Sample ID` = paste0("DMR_", 1:nrow(wide_data_DMR2)))
...
class(wide_data_DMR4$`Sample date`)
...
wide_data_DMR5 <- wide_data_DMR4 |> select(all_of(desired_order), everything())

# 10: Final misc. cleaning ####

# Remove Total PFAS values from
wide_data_DMR6  <- wide_data_DMR5  %>% select(-`Total PFOA and PFOS`)


# Subset wastewater and groundwater
Permits_DMR_Groundwater_Date <- wide_data_DMR6 %>% filter(Notes == "General Permit Covered Facility")
Permits_DMR_Wastewater_Date <- wide_data_DMR6 %>% filter(Notes == "NPDES Individual Permit")
...
write_xlsx(
  ...
  "03_Clean_Data/Wastewater/Permits_DMR_Wastewater_Date.xlsx"
)
