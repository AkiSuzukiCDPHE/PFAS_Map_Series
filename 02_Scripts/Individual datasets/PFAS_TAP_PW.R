...
PFAS_TAP_Pilot_Wide_1 <- PFAS_TAP_Pilot_Wide |>
  mutate(across(All_Analytes, ~ dplyr::if_else(is.na(.), 0.0, .))) |> select(-SAMPLE_ADDRESS)
...
PFAS_TAP_All_2 <- PFAS_TAP_All_1 %>%
  ...
  mutate(
    ...
    Link = "https://cdphe.colorado.gov/pfas/pfas-health/pfas-testing",
    Program = "The Toxicology and Risk Assessment Unit (TARA)",
    `Number of samples` = 1,
    Units = "ng/L"
  )
